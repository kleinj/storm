...
template <typename SparseValueTypeT, typename DenseValueTypeT, typename IntermediateValueTypeT = DenseValueTypeT>
class StandardWorkerOtherValueType {
public:
    typedef SparseValueTypeT SparseValueType;
    typedef DenseValueTypeT DenseValueType;
    typedef IntermediateValueTypeT IntermediateValueType;
    ...
    IntermediateValueType solve(const Matrix& triangularMatrix, std::size_t row, std::vector<IntermediateValueType>& vector) {
        IntermediateValueType result;
        for (std::size_t j = row + 1; j < triangularMatrix.getColumnCount() - 1; j++) {
            result -= converter.convert(triangularMatrix(row, j)) * vector.at(j);
        }
        result += converter.convert(triangularMatrix(row, triangularMatrix.getColumnCount()-1));
        result /= converter.convert(triangularMatrix(row, row));
        return result;
    }
    ...
    IntermediateValueType solve_sparse(const Matrix& triangularMatrix, std::size_t row, std::vector<IntermediateValueType>& vector) {
        ...
        for (auto& entry : triangularMatrix.row_col_range(row, row+1, b_col-1)) {
            ...
            result -= converter.convert(entry.getValue()) * vector.at(j);
        }
        result += converter.convert(triangularMatrix.getValueRight(row, b_col));
        result /= converter.convert(triangularMatrix.getValueLeft(row, row));
        return result;
    }
    ...
    static std::string getMethodName() {
        if (std::is_same<SparseValueType, IntermediateValueType>::value) {
            return std::string("Standard [matrix entries/back-substitution: ") + storm::utility::NumericalTypes::getTypeName<DenseValueType>() + ", final result: " + storm::utility::NumericalTypes::getTypeName<SparseValueType>()+ "]";
        } else {
            return std::string("Standard [matrix entries: ") + storm::utility::NumericalTypes::getTypeName<DenseValueType>() + ", back-substitution: " + storm::utility::NumericalTypes::getTypeName<IntermediateValueType>() + ", final result: " + storm::utility::NumericalTypes::getTypeName<SparseValueType>()+ "]";
        }
    }
...
private:
    DenseValueType pivot;
    storm::utility::NumericalTypesConverter<DenseValueType, IntermediateValueType> converter;
    bool debug = false;
};
